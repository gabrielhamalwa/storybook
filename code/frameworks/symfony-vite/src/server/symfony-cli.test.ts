...
describe('startSymfonyCliServer', () => {
  beforeEach(() => {
  ...
  it('spawns symfony server:start with the correct arguments and environment', async () => {
    const server = await startSymfonyCliServer({
    ...
    expect(spawn).toHaveBeenCalledWith('symfony', ['server:start', '--port=12345', '--no-tls'], {
      cwd: '/project',
